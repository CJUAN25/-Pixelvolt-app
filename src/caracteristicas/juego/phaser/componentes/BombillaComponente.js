...
class BombillaComponente extends Phaser.GameObjects.Container {
  ...
  eliminarCablesConectados(scene) {
    if (scene.conexionesPorComponente.has(this)) {
      ...
      console.log(`🗑️ Bombilla eliminada con ${cablesConectados.size} cables conectados`);
    }
  }
  
  /**
   * Enciende la bombilla visualmente (tinte amarillo)
   */
  encender() {
    if (this.imagenPrincipal) {
      this.imagenPrincipal.setTint(0xffff00); // Amarillo brillante
      // TODO: Añadir animación de brillo o partículas
    }
  }
  
  /**
   * Apaga la bombilla visualmente (sin tinte)
   */
  apagar() {
    if (this.imagenPrincipal) {
