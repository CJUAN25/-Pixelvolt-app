...
/**
 * Componente Base - Clase padre para todos los componentes interactivos del juego
 * Hereda de Phaser.GameObjects.Container y encapsula la lógica común.
 * 
 * Todos los componentes del juego (Batería, Bombilla, Interruptor, Resistencia, etc.)
 * heredan de esta clase base que proporciona:
 * - Constructor con creación de imagen y puntos de conexión
 * - Configuración de interactividad y arrastre
 * - Doble clic para eliminar componente
 * - Eliminación automática de cables conectados
 */
class ComponenteBase extends Phaser.GameObjects.Container {
  /**
  ...
  eliminarCablesConectados(scene) {
    if (scene && scene.conexionesPorComponente && scene.conexionesPorComponente.has(this)) {
      ...
      const nombreComponente = this.getData('herramientaInfo')?.nombre || 'Componente';
      console.log(`🗑️ ${nombreComponente} eliminado con ${cablesAEliminar.length} cables conectados`);
    }
  }
}
